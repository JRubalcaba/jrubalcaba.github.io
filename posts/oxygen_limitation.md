...
The empirical and modelled results together suggest that oxygen limitation determines how metabolic rates change with temperature and body mass, supporting the model's predictions. Of course, similarity between observed and predicted patterns doesn't mean that oxygen limitation is the ultimate mechanisms driving the empirical results. Our model does not reflect an underpinning phyisical law that all fish obey, because biological systems are complex, evolve, and respond to environmental changes. Being pragmatic, our model is useful because it provides a means to calculate the effect of oxygen limitation and the role of compensatory mechanisms to mitigate suffocation (such as gill ventilation, reductions in activity level, or physiological adaptations to increase the affinity of respiratory pigments for oxygen molecules).

Our study demonstrates that warming waters can cause respiratory distress in fish impairing their physiological performance. Further experimental research is necesary to understand the impact of oxygen limitation on fish physiology, behavior and survival. 
